...
logging.basicConfig(level = logging.DEBUG,
                    ...
                    handlers=[logging.FileHandler('log_precio_claros.log'), 
...
def carga_productos():
	...
	fproductos[['factor_homogenea','um_homogenea']] = fproductos.apply(lambda x: homogenea(float(x['cantidad_depurada']),str(x['um_depurada'])),axis=1, result_type='expand')

	# Creamos variables Dummies que identifiquen el tipo de Unidad de Medida
	fproductos=pd.get_dummies(fproductos, columns=['um_homogenea'],prefix='dummy_um_homogenea',drop_first=True)
	...
	dummy = pd.get_dummies(fproductos.marcas_frecuentes,prefix='dummy_marca',drop_first=True)
...
def carga_sucursales():
	...
	fsucursales = fsucursales[fsucursales.sucursalTipo !='Minorista']

	column_lower(fsucursales,columns=['nom_provincia','sucursalTipo','region'])
	
	remove_accents(fsucursales,columns=['nom_provincia','sucursalTipo','region'])

	sucursales_p=pd.get_dummies(fsucursales, columns=['nom_provincia'],prefix='dummy_provincia',drop_first=True)
	sucursales_new = pd.merge(fsucursales,sucursales_p)

	sucursales_p=pd.get_dummies(sucursales_new, columns=['sucursalTipo'],prefix='dummy_sucursal_tipo',drop_first=True)
	sucursales_new = pd.merge(sucursales_new,sucursales_p)

	sucursales_p=pd.get_dummies(sucursales_new, columns=['region'],prefix='dummy_region',drop_first=True)
	sucursales_new = pd.merge(sucursales_new,sucursales_p)
	
	sucursales_new['sucursal_id_2'] = sucursales_new.id.str.replace('-','',regex=True)

	sucursales_new.columns = sucursales_new.columns.str.replace(' ','_',regex=True)
...
def optimizarDatos(p_df):
	...
	p_df['producto_id'] = p_df.producto_id.str.replace('-','',regex=True)
...
def guardarArchivo(p_df):
	pd.to_pickle(p_df, 'sucursal_producto_precio_v2.pkl')#, compression="zip")
	logger.info('Finish guardado de archivo pkl')

def guardarArchivo_csv(p_df):
	p_df.to_csv('sucursal_producto_precio_v2.csv',sep='|',index=False)
	logger.info('Finish guardado de archivo csv')

def obtenerPriorPrecioRelativo(p_df):
	logger.info('Obteniendo precio relativo anterior')
	gsort=p_df.sort_values(by=['producto_id','sucursal_id','fecha'],ascending=False)
	...
	p_df = pd.merge(p_df,aux,how='left')

	gsort_media=p_df.sort_values(by=['nom_provincia','producto_id','fecha'],ascending=False)
	...
	p_df = pd.merge(p_df,aux_media,how='left')
	p_df['priorPrecioRelativo'].fillna(p_df['priorMedia']/p_df['bienNumerico'],inplace=True)	

	p_df['priorPrecioRelativo'].fillna(p_df['media']/p_df['bienNumerico'],inplace=True)
	logger.info('Finish obtenerPriorPreccioRelativo')
...
def main():
	...
	#Obtenemos el precioRelativo del periodo T-1
	datasetNew=obtenerPriorPrecioRelativo(datasetNew)
